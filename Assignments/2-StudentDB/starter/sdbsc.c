...
int get_student(int fd, int id, student_t *s)
{
    ...
    // move pointer and read to check student records
    if (lseek(fd, offset, SEEK_SET) == -1 || read(fd, s, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE)
    {
        return ERR_DB_FILE;
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // Check if the record slot is already occupied
...
int del_student(int fd, int id)
{
    ...
    // Use get_student() to check if the student exists
    int result = get_student(fd, id, &student);

    if (result == ERR_DB_FILE)
    {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE; // Error reading file
    }
    ...
    // Write an empty student record to "delete" the student
...
int count_db_records(int fd)
{
    ...
    // Check for read error (other than reaching EOF)
    if (bytes_read == -1)
    {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
...
int print_db(int fd)
{
    ...
    while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE)
    {
        ...
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0)
        {
            if (!record_found)
            {
                // Print header only once, when first valid record is found
    ...
    // If no valid records were found, print a message
    if (!record_found)
    {
        printf(M_DB_EMPTY);
    }
...
{
    // Validate that the student pointer is not NULL and the student ID is not zero
    if (s == NULL || s->id == 0)
    {
